...
\begin{abstract}
\noindent PCOS Pathfinder is a data-grounded clinical decision-support prototype for earlier identification of Polycystic Ovary Syndrome (PCOS) and broader differential workup in women's health. The system anchors on the required PCOS clinical dataset (n=541, Kerala, India), layers a synthetic endometriosis-overlap module for differential reasoning, and uses a lightweight single-cell gene-list check for biological rationale. The delivered artifacts are thirteen reproducible notebooks, three saved scikit-learn models, a Streamlit clinician interface, exported metrics and figures, and this report. Thresholds are selected from out-of-fold training probabilities (no test leakage), risk tiers are anchored to each model's action threshold, and patient-level SHAP attributions are provided for both PCOS models. On the held-out test split the enhanced model reaches ROC-AUC 0.953 with recall 0.886 and specificity 0.902. The system is framed throughout as decision support, not standalone diagnosis.
\end{abstract}
...
Data cleaning is implemented in the notebook setup cells generated from the deterministic regenerator script (\code{scripts/create\_training\_notebooks.mjs}). The key steps:
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \code{blood\_group}: excluded from the current model inputs because codes 11--18 are categorical ABO/Rh labels, not an ordered clinical scale.
...
\subsection{Blood Group Handling: Categorical Metadata, Not Ordinal Diagnosis}

Dropping \code{blood\_group} from the current models is an encoding and evidence-boundary choice, not a claim that blood group is irrelevant. The linked 2025 BMC Endocrine Disorders cross-sectional study reported no statistically significant ABO distribution difference between PCOS and controls, so blood group should not be treated as a standalone PCOS diagnostic feature. Within PCOS patients, however, the same study found blood type associated with menstrual bleeding level and differences in BMI, FSH, LH, and estradiol \href{https://link.springer.com/article/10.1186/s12902-025-01898-0}{(Yang et al., 2025)}. In future iterations, the app should retain validated ABO/Rh blood group as categorical metadata for phenotype and symptom-severity stratification (for example, one-hot encoded or used for subgroup displays), while never feeding the numeric 11--18 codes as an ordinal variable.
...
A single stratified train/test split with \code{test\_size=0.25} and \code{random\_state=42}. Model selection uses 5-fold stratified cross-validation on the \emph{training} split. For each chosen model, action thresholds are selected using out-of-fold training probabilities from \code{cross\_val\_predict(method="predict\_proba")}. The threshold sweep targets recall $\geq 0.90$ on the training out-of-fold predictions while maximising specificity, then precision, then F1.

The held-out test set is then evaluated \textbf{exactly once}. This eliminates the earlier risk of choosing a threshold on the test set and reporting optimistic metrics.

\section{Methodology Rigor Layer}

The base modelling pipeline is deliberately compact, but the submission adds a separate validation layer to make the clinical claims harder to dismiss. Each check is reproducible from a numbered notebook and is reported as evidence, not decoration.

\begin{table}[H]
\centering
\caption{Rigor analyses added beyond base model training}
\small
\begin{tabularx}{\linewidth}{p{0.13\linewidth}p{0.32\linewidth}X}
\toprule
Notebook & Analysis & What it adds to the submission \\
\midrule
07 & Bootstrap uncertainty & 2000-resample confidence intervals on every held-out metric; paired AUC comparison suggests the enhanced model improves AUC by $\Delta=+0.057$ (95\% CI $[+0.013,+0.105]$, $p\approx0.015$). \\
08 & Calibration & Brier score and expected calibration error before and after Platt/isotonic recalibration. Platt scaling improves enhanced Brier from 0.093 to 0.072 and ECE from 0.143 to 0.045. \\
09 & Decision Curve Analysis & Net-benefit curves versus treat-all and treat-none baselines; both chosen action thresholds sit inside useful threshold ranges. \\
10 & TabPFN benchmark & A contemporary small-tabular foundation model reaches similar AUC, but the random forest keeps better recall/F2 at the selected clinical threshold and is simpler to deploy. \\
11 & Subgroup fairness audit & Recall is stratified by age band and BMI category with bootstrap intervals. The screening model has a 25-point BMI recall gap, flagged as a deployment risk. \\
12 & Conformal prediction & Split-conformal prediction sets target 90\% coverage and achieve 0.912 empirical coverage on holdout; empty sets are treated as abstentions requiring clinician review. \\
13 & Rotterdam alignment & A hand-coded Rotterdam-style 2-of-3 comparator is benchmarked on the same holdout. The enhanced ML model adds 9.1 points sensitivity at a 1.1 point specificity cost, with the caveat that testosterone is unavailable. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{subfigure}{0.48\linewidth}
  \includegraphics[width=\linewidth]{calibration_combined.png}
  \caption{Calibration before and after recalibration.}
\end{subfigure}\hfill
\begin{subfigure}{0.48\linewidth}
  \includegraphics[width=\linewidth]{dca_combined.png}
  \caption{Decision curve net benefit.}
\end{subfigure}
\caption{Clinical-utility checks for probability quality and actionability.}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Subgroup recall gap.} Notebook 11 finds lower screening recall for the normal-BMI subgroup. The prototype therefore warns clinicians not to rule out lean PCOS solely from a low screening probability.
  \item \textbf{Clinical framing.} All outputs are framed as risk, triage, and decision support, not diagnosis.
  \item \textbf{Version sensitivity.} scikit-learn model artifacts are sensitive to version skew; \code{requirements.txt} pins the core artifact-loading stack and constrains TabPFN to the open-weights checkpoint line used for notebook 10.
...
\code{notebooks/07\_uncertainty\_quantification.ipynb} & Bootstrap confidence intervals and paired AUC comparison. \\
\code{notebooks/08\_calibration\_analysis.ipynb} & Calibration curves, Brier score, ECE, Platt and isotonic recalibration. \\
\code{notebooks/09\_decision\_curve\_analysis.ipynb} & Decision-curve net-benefit analysis. \\
\code{notebooks/10\_tabpfn\_benchmark.ipynb} & TabPFN-v2 benchmark against the selected random forest pipeline. \\
\code{notebooks/11\_subgroup\_fairness.ipynb} & Age-band and BMI-category recall audit with bootstrap intervals. \\
\code{notebooks/12\_conformal\_prediction.ipynb} & Split-conformal prediction-set coverage check. \\
\code{notebooks/13\_rotterdam\_alignment.ipynb} & Rotterdam-style clinical-rule comparison on the same holdout split. \\
